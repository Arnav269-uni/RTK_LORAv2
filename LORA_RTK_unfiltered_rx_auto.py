"""
RTK_Lora_RX_to_FC.py
---------------------
Matches RTK_Lora_TX.py's wire format over LoRa (ASCII text, not raw
binary):
...
There is NO epoch buffering here - each individual RTCM3 message is
sent to the flight controller via pymavlink (GPS_RTCM_DATA) the instant
it's received (and reassembled, for split messages like 1124):

    TX --1074(msg)(via LoRa)--> this script --pymavlink--> FC

Messages <=180 bytes go out as a single GPS_RTCM_DATA packet. Messages
>180 bytes (in practice, BeiDou 1124 with a full satellite set can
reach ~187 bytes) are fragmented per the MAVLink spec - see
send_rtcm_to_fc() for exactly how.

LoRa link parameters (must match RTK_Lora_TX.py exactly):
    Band       : 865,000,000 Hz
    Network ID : 5
    UART baud  : 57600
    Air params : SF9, BW 500kHz, CR 4/5, Preamble 12
                 (AT+PARAMETER=9,9,1,12)
...
class RYLR998:
    def _auto_config_lora(self, port):
        """
        Auto-detect and fully configure the RX LoRa module.
        Hardcoded from compatibility test results:
            ADDRESS=200, NETWORKID=5, BAND=865000000, PARAMETER=7,9,1,12, BAUD=115200
        """
...
def main():
    ...
    device_id = "200"
    lora_port = input("LoRa COM port (e.g. COM4): ").strip()
    ...
    # [FIX] Removed duplicate configure call. _auto_config_lora inside __init__ already does this.
