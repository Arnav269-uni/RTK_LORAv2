    ...
    def _accept_loop(self):
        while self.running:
            try:
                ...
                print(f"[tcp] Follower connected from {client_addr}")
    ...
    def broadcast(self, data: bytes):
        with self.clients_lock:
            ...
            for dead in dead_clients:
                print("[tcp] Follower disconnected")
...
# ==================== Main ====================
def main():
    device_id = input("Enter this device's LoRa address/ID (0-65535): ").strip()
    ...
    tcp_server = TcpRtcmServer(TCP_BIND_ADDR, TCP_PORT)
    ...
    try:
        while True:
            ...
            num_followers = tcp_server.broadcast(raw)
            followers_str = f" + broadcast to {num_followers} follower(s)" if num_followers > 0 else ""

            if result is None:
                print(f"  [{msg_type}] {len(raw)} bytes - DROPPED (exceeds max fragmentable size){followers_str}")
            elif len(result) == 1:
                flags, length = result[0]
                print(f"  [{msg_type}] {len(raw)} bytes -> sent to FC (1 packet, {length}B) - ack{followers_str}")
            else:
                parts = ", ".join(f"frag{((f>>1)&0x3)}:{l}B" for f, l in result)
                print(f"  [{msg_type}] {len(raw)} bytes -> sent to FC "
                      f"({len(result)} fragments: {parts}) - ack{followers_str}")
